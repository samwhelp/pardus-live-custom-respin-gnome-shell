...
TARGET_ARCH="amd64"
TARGET_NAME="pardus-gnome-shell"
TARGET_BUSINESS_NAME="Pardus-Gnome-Shell"
TARGET_HOSTNAME="pardus-gnome-shell"
TARGET_BUILD_VERSION="25"
...
TARGET_PARDUS_CODENAME="yirmibes"
TARGET_PARDUS_MIRROR="http://depo.pardus.org.tr/pardus"
...
TARGET_PARDUS_CODENAME="${TARGET_PARDUS_CODENAME}"
TARGET_PARDUS_MIRROR="${TARGET_PARDUS_MIRROR}"
...
function core_var_dump () {
	...
	echo "TARGET_PARDUS_CODENAME=\${TARGET_PARDUS_CODENAME}"
	echo "TARGET_PARDUS_MIRROR=\${TARGET_PARDUS_MIRROR}"
...
function core_var_export () {
	...
	export TARGET_PARDUS_CODENAME
	export TARGET_PARDUS_MIRROR
...
##
## ## Module / Apt Sources / Pardus
##

function core_apt_sources_config_for_pardus () {

	echo "################################################################################"
	echo "## [Worker] core_apt_sources_config_for_pardus"
	echo "################################################################################"

	echo "==== config apt soruces list for pardus ===="

	sys_pardus_add_keyring

	sys_pardus_add_apt_sources

	##sys_pardus_add_apt_preferences

}

function sys_pardus_add_keyring () {

	echo "==== sys_pardus_add_keyring ===="

	local keyring_deb_file_name="pardus-archive-keyring_2025.1_all.deb"

	echo 'apt-get install -y --install-recommends gnupg wget'
	apt-get install -y --install-recommends gnupg wget

	mkdir -p "/tmp"
	echo "wget -c http://depo.pardus.org.tr/pardus/pool/main/p/pardus-archive-keyring/\${keyring_deb_file_name} -O /tmp/\${keyring_deb_file_name}"
	wget -c "http://depo.pardus.org.tr/pardus/pool/main/p/pardus-archive-keyring/\${keyring_deb_file_name}" -O "/tmp/\${keyring_deb_file_name}"
...
function sys_pardus_add_apt_sources () {

	echo "==== sys_pardus_add_apt_sources ===="

	local target_pardus_codename="\${TARGET_PARDUS_CODENAME}"
	local target_pardus_mirror="\${TARGET_PARDUS_MIRROR}"
	local target_arch="\${TARGET_ARCH}"

	mkdir -p "/etc/apt/sources.list.d"

cat << __EOF__ | tee "/etc/apt/sources.list.d/pardus.sources"
Types: deb
URIs: \${target_pardus_mirror}
Suites: \${target_pardus_codename}
Components: main contrib non-free non-free-firmware
Signed-By: /usr/share/keyrings/pardus-archive-keyring.gpg
__EOF__


}

function sys_pardus_add_apt_preferences () {

	echo "==== sys_pardus_add_apt_preferences ===="

	mkdir -p "/etc/apt/preferences.d"

cat << __EOF__ | tee "/etc/apt/preferences.d/pardus.pref"  > /dev/null 2>&1
Package: *
Pin: origin depo.pardus.org.tr
Pin-Priority: 750

Package: *
Pin: release o=pardus,c=upstream
Pin-Priority: 700
__EOF__
...
function pardus_base_package_install () {

	echo "################################################################################"
	echo "## [Worker] pardus_base_package_install"
	echo "################################################################################"

	echo "==== install pardus base packages ===="
...
function model_do_fulfill_scripts () {
	...
	core_apt_sources_config_for_pardus
	core_apt_update
	pardus_base_package_install
	core_apt_upgrade
